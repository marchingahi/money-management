...
import type { Account, PlannedExpense, Transfer, FixedCost, Income, Member, PaymentMethod, Settings, Transaction } from './types'
...
export function convertLegacy(legacy: LegacyData, uuid: () => string = () => crypto.randomUUID()): ConvertResult {
  ...
  return {
    data: { members, incomes, accounts: [], transfers: [], planned: [], methods, fixedCosts, transactions, settings: legacy.settings },
    deletedSeedIds: {
